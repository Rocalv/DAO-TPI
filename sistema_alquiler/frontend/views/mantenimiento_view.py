...
from sistema_alquiler.backend.models.empleado import Empleado # <-- NUEVO IMPORT
...
class RegistrarMantenimientoView(tk.Frame):
    ...
    def create_widgets(self):
        """Crea los 3 componentes: Filtro, Tabla y Formulario."""
        ...
        self.form_mecanico_var = tk.StringVar() # <-- NUEVA VARIABLE

        # --- Columna 1 ---
        tk.Label(self.form_frame, text="Vehículo:", bg=BG_COLOR, fg=FG_COLOR).grid(row=0, column=0, sticky="e", padx=5, pady=2)
        tk.Entry(self.form_frame, textvariable=self.form_patente_var, width=15, state="disabled", disabledbackground=ENTRY_BG, disabledforeground=ENTRY_FG).grid(row=0, column=1, padx=5, pady=2, sticky="w")
        tk.Entry(self.form_frame, textvariable=self.form_modelo_var, width=25, state="disabled", disabledbackground=ENTRY_BG, disabledforeground=ENTRY_FG).grid(row=0, column=2, padx=5, pady=2, sticky="w")
        
        tk.Label(self.form_frame, text="Kilometraje Actual:", bg=BG_COLOR, fg=FG_COLOR).grid(row=1, column=0, sticky="e", padx=5, pady=2)
        tk.Entry(self.form_frame, textvariable=self.form_km_var, width=15, state="disabled", disabledbackground=ENTRY_BG, disabledforeground=ENTRY_FG).grid(row=1, column=1, padx=5, pady=2, sticky="w")
        
        # --- NUEVO WIDGET ---
        tk.Label(self.form_frame, text="Mecánico Asignado:", bg=BG_COLOR, fg=FG_COLOR).grid(row=2, column=0, sticky="e", padx=5, pady=2)
        self.combo_mecanicos = ttk.Combobox(self.form_frame, textvariable=self.form_mecanico_var, state="readonly", width=41)
        self.combo_mecanicos.grid(row=2, column=1, columnspan=2, padx=5, pady=2, sticky="w")
        
        self.form_frame.grid_columnconfigure(3, pad=30)

        # --- Columna 2 ---
        tk.Label(self.form_frame, text="Servicio a Realizar:", bg=BG_COLOR, fg=FG_COLOR).grid(row=0, column=4, sticky="e", padx=5, pady=2)
        self.combo_servicios = ttk.Combobox(self.form_frame, textvariable=self.form_servicio_var, state="readonly", width=30)
        self.combo_servicios.grid(row=0, column=5, padx=5, pady=2, sticky="w")

        tk.Label(self.form_frame, text="Proveedor:", bg=BG_COLOR, fg=FG_COLOR).grid(row=1, column=4, sticky="e", padx=5, pady=2)
        tk.Entry(self.form_frame, textvariable=self.form_proveedor_var, width=32, bg=ENTRY_BG, fg=ENTRY_FG, insertbackground=FG_COLOR).grid(row=1, column=5, padx=5, pady=2, sticky="w")
        
        tk.Label(self.form_frame, text="Descripción:", bg=BG_COLOR, fg=FG_COLOR).grid(row=3, column=0, sticky="e", padx=5, pady=10)
        self.form_desc_entry = tk.Entry(self.form_frame, bg=ENTRY_BG, fg=ENTRY_FG, insertbackground=FG_COLOR)
        self.form_desc_entry.grid(row=3, column=1, columnspan=5, padx=5, pady=10, sticky="ew")
        
        self.form_frame.grid_columnconfigure(6, pad=30)
        tk.Button(self.form_frame, text="Registrar Mantenimiento", command=self.controller.registrar_mantenimiento, bg=BTN_BG, fg=BTN_FG, height=2).grid(row=0, column=7, rowspan=4, padx=20, sticky="ns")
    ...
    def set_mecanicos_combobox(self, mecanicos: List[Empleado]):
        """Carga el combobox de mecánicos."""
    ...
    def limpiar_formulario(self):
        ...
        self.form_mecanico_var.set("") # <-- CAMBIO
